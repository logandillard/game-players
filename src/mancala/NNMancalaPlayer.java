...
import games.NNPlayer;
import games.NoLookNNPlayer;
...
//		throw new RuntimeException("Stones on board don't add up");
		...
		if(scoreDiff != 0) {
			...
			for(int i=0; i<4; i++) {
				...
				if(scoreDiff > 4) {
					...
					tempScoreDiff = (double)scoreDiff;
				}
		...
		for(int p=0; p<2; p++) {
			closeScore = (double)closeToWinning[p];
